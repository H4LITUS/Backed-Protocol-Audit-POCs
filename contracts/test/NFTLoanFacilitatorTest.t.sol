...
contract NFTLoanFacilitatorGasBenchMarkTest is DSTest {
    ...
    function setUp() public {
        ...
        facilitator.lend(2, interestRate, loanAmount, loanDuration, address(this));
    }
    ...
    function testLend() public {
        facilitator.lend(1, interestRate, loanAmount, loanDuration, address(this));
    }

    function testLendBuyout() public {
        facilitator.lend(2, interestRate, loanAmount + ((loanAmount * 10) / 100), loanDuration, address(this));
    }
...
contract NFTLoanFacilitatorTest is DSTest {
    ...
    function testCreateLoanEmitsCorrectly() public {
        ...
        emit CreateLoan(1, borrower, punkId, address(punks), interestRate, address(dai), loanAmount, loanDuration);
        vm.prank(borrower);
        facilitator.createLoan(punkId, address(punks), interestRate, loanAmount, address(dai), loanDuration, borrower);
    }
    ...
    function testCreateLoanZeroDurationNotAllowed() public {
        ...
        facilitator.createLoan(punkId, address(punks), interestRate, loanAmount, address(dai), 0, borrower);
    }

    function testCreateLoanZeroAmountNotAllowed() public {
        vm.startPrank(borrower);
        vm.expectRevert("NFTLoanFacilitator: 0 loan amount");
        facilitator.createLoan(punkId, address(punks), interestRate, 0, address(dai), loanDuration, borrower);
    }

    function testCreateLoanAddressZeroCollateralFails() public {
        vm.startPrank(borrower);
        vm.expectRevert(bytes(""));
        facilitator.createLoan(punkId, address(0), interestRate, loanAmount, address(dai), loanDuration, borrower);
    }
    ...
    function testClosingLoanWithLender() public {
        ...
        facilitator.lend(loanId, interestRate, loanAmount, loanDuration, borrower); // have borrower lend, this is not realistic, but will do for this test

        // loan has lender, should now revert
        vm.expectRevert("NFTLoanFacilitator: has lender, use repayAndCloseLoan");
        facilitator.closeLoan(loanId, borrower);
    }
    ...
    function testLendUpdatesValuesCorrectly(uint16 rate, uint128 amount, uint32 duration, address sendTo) public {
    ...
    function testLendEmitsCorrectly() public {
        ...
        emit Lend(loanId, address(this), interestRate, loanAmount, loanDuration);

        facilitator.lend(loanId, interestRate, loanAmount, loanDuration, address(1));
    }
    ...
    function testSuccessfulLend() public {
        ...
        facilitator.lend(loanId, interestRate, loanAmount, loanDuration, lender);
        (, , , uint40 lastAccumulatedTimestamp, , , uint256 accumulatedInterest, , ) = facilitator.loanInfo(loanId);
        assertEq(lastAccumulatedTimestamp, startTimestamp);
        assertEq(accumulatedInterest, 0);
    ...
    function testBuyoutUpdatesValuesCorrectly() public {
        ...
        vm.prank(newLender);
        facilitator.lend(loanId, interestRate, loanAmount, newDuration, address(1));
        (
            bool closed,
            uint16 interest,
            uint32 durationSeconds,
    ...
    function testBuyoutTransfersLendTicket() public {
        ...
        vm.prank(newLender);
        facilitator.lend(loanId, interestRate, loanAmount, newDuration, newLender);
    ...
    function testBuyoutPaysPreviousLenderCorrectly(uint128 amount) public {
        ...
        facilitator.lend(loanId, interestRate, amount, uint32(increaseByMinPercent(loanDuration)), address(1));
    ...
    function testBuyoutFailsIfTermsNotImproved() public {
        ...
        setUpLender(newLender);
        vm.startPrank(newLender);
        vm.expectRevert("NFTLoanFacilitator: proposed terms must be better than existing terms");
        facilitator.lend(loanId, interestRate, loanAmount, loanDuration, newLender);
    }
    ...
    function testBuyoutFailsIfLoanAmountNotSufficientlyImproved() public {
        ...
        vm.expectRevert("NFTLoanFacilitator: proposed terms must be better than existing terms");
        facilitator.lend(loanId, interestRate, uint128(newAmount), loanDuration, newLender);
        vm.stopPrank();
    }
    ...
    function testBuyoutFailsIfLoanAmountRegressed(uint16 newRate, uint32 newDuration, uint128 newAmount) public {
        ...
        facilitator.lend(loanId, newRate, uint128(newAmount), newDuration, newLender);
        vm.stopPrank();
    }
    ...
    function testRepayAndCloseSuccessful() public {
        ...
        assertEq(dai.balanceOf(borrower), balanceOfBorrower - (loanAmount + interestAccrued));
    ...
    function testUpdateOriginationFeeRevertsIfGreaterThanFivePercent() public {
        ...
        facilitator.updateOriginationFeeRate(uint32(6 * (10 ** (interestRateDecimals - 2))));
    }

    function testUpdateOriginationFeeWorks() public {
        uint256 interestRateDecimals = facilitator.INTEREST_RATE_DECIMALS();
        vm.startPrank(address(this));
        facilitator.updateOriginationFeeRate(uint32(2 * (10 ** (interestRateDecimals - 2))));
        assertEq(facilitator.originationFeeRate(), uint32(2 * (10 ** (interestRateDecimals - 2))));
    }

    function testUpdateRequiredImprovementRateRevertsIfNotCalledByManager() public {
        vm.startPrank(address(1));
        vm.expectRevert("Ownable: caller is not the owner");
    ...
    function testUpdateRequiredImprovementRateWorks() public {
        ...
        assertEq(facilitator.requiredImprovementRate(), 20 * facilitator.SCALAR());
    }
