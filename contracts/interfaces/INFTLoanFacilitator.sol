...
interface INFTLoanFacilitator {
    ...
    struct Loan {
        bool closed;
        uint16 perAnumInterestRate; // set
        uint32 durationSeconds; // set - updated in lend()
        uint40 lastAccumulatedTimestamp; // set (lend)
        address collateralContractAddress; // set
        address loanAssetContractAddress; // set
        uint128 accumulatedInterest;
        uint128 loanAmount; // set - updated in lend()
        uint256 collateralTokenId; // set
    }
     ...
     */
    event Lend(
        uint256 indexed id, address indexed lender, uint256 interestRate, uint256 loanAmount, uint256 durationSeconds
    );
     ...
     */
    function lend(uint256 loanId, uint16 interestRate, uint128 amount, uint32 durationSeconds, address sendLendTicketTo)
        external;
     ...
     * @param loanId The loan id
     */
    function loanEndSeconds(uint256 loanId) external view returns (uint256);
}
