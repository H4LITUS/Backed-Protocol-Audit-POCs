...
library PopulateSVGParams {
        ...
        returns (NFTLoanTicketSVG.SVGParams memory)
    {
        ...
        ) = nftLoanFacilitator.loanInfo(id);
        ...
        svgParams.endDateTime =
            lastAccumulatedTimestamp == 0 ? "n/a" : endDateTime(lastAccumulatedTimestamp + durationSeconds);
    ...
    {
        return UintStrings.decimalString(perAnumInterestRate, nftLoanFacilitator.INTEREST_RATE_DECIMALS() - 2, true);
    }
        ...
        returns (string memory)
    {
        return UintStrings.decimalString(
            nftLoanFacilitator.interestOwed(loanId), IERC20Metadata(loanAsset).decimals(), false
        );
    }
    ...
    function endDateTime(uint256 endDateSeconds) private pure returns (string memory) {
        (uint256 year, uint256 month, uint256 day, uint256 hour, uint256 minute, uint256 second) =
            BokkyPooBahsDateTimeLibrary.timestampToDateTime(endDateSeconds);
