...
contract NFTLoanFacilitator is Ownable, INFTLoanFacilitator {
    ...
    uint256 public constant override SCALAR = 10 ** INTEREST_RATE_DECIMALS; // 10**3 => 1000

    // ==== state variables ====

    /// See {INFTLoanFacilitator-originationFeeRate}.
    /// @dev starts at 1%
    uint256 public override originationFeeRate = 10 ** (INTEREST_RATE_DECIMALS - 2); // 10**(3-2) => 10

    /// See {INFTLoanFacilitator-requiredImprovementRate}.
    /// @dev starts at 10%
    uint256 public override requiredImprovementRate = 10 ** (INTEREST_RATE_DECIMALS - 1); // 10**(3-1) => 100
    ...
    constructor(address _manager) {
        ...
        // @audit Why aren't the lendTicketContract and borrowTicketContract set here?
        // rather they are set later by the owner and can be changed by the owner anytime
    }
    ...
    function createLoan(
        ...
        require(
            collateralContractAddress != lendTicketContract,
        ...
        // @audit This contract address can be any malicious contract
        ...
        // @audit-info If the collateralContractAddress is malicious (noted above), the holder will still receive a borrow ticket contract without depositing any NFT
        // @audit There should be a check that the address "mintBorrowTicketTo" supports ERC720 tokens? or maybe is an EOA?
    ...
    function closeLoan(uint256 loanId, address sendCollateralTo) external override notClosed(loanId) {
        require(
            IERC721(borrowTicketContract).ownerOf(loanId) == msg.sender,
        ...
        loan.closed = true;
        // @audit IERC721.safeTransferFrom should be used
        // If the receiving address is a contract that doesn't support ERC-721, then the NFT would be permanently lost
    ...
    function lend(
        ...
        if (loan.lastAccumulatedTimestamp == 0) {
            ...
            loan.lastAccumulatedTimestamp = uint40(block.timestamp); // @audit-info to check later
...

            {
                ...
                require(
                    ((previousLoanAmount * requiredImprovementRate) / SCALAR) <= amountIncrease ||
                        previousDurationSeconds + ((previousDurationSeconds * requiredImprovementRate) / SCALAR) <=
                        durationSeconds ||
                        (previousInterestRate != 0 && // do not allow rate improvement if rate already 0
                            previousInterestRate - ((previousInterestRate * requiredImprovementRate) / SCALAR) >=
                            interestRate),
            ...
            require(
                accumulatedInterest <= type(uint128).max,
            ...
            loan.durationSeconds = durationSeconds;
            loan.loanAmount = amount;
            // @audit The accumulated interest (uint256 here) can overflow in the loan struct (uint128)
    ...
    function repayAndCloseLoan(uint256 loanId) external override notClosed(loanId) {
        ...
        // @audit permanent loss of user funds.
        // If the owner of borrowTicket doesn't implement the IERC721Receiver interface,
        // the transfer will fail, the NFT will get stuck in this contract until the expiry of the loan time
        // Neither the lender can recover funds nor the borrower can get the NFT back
    ...
    function seizeCollateral(uint256 loanId, address sendCollateralTo) external override notClosed(loanId) {
        require(
            IERC721(lendTicketContract).ownerOf(loanId) == msg.sender,
            "NFTLoanFacilitator: lend ticket holder only"
        );

        Loan storage loan = loanInfo[loanId];
        require(
            block.timestamp > loan.durationSeconds + loan.lastAccumulatedTimestamp,
    ...
    function setLendTicketContract(address _contract) external onlyOwner {
        ...
        // @audit check for _contract != address(0) is missing
        // @audit what if the owner changes the lendingTicketContract?
        // Lenders won't be able to recover sieze collateral
        // During repayAndCloseLoan(), the ERC20 tokens will be sent to the zero or some other address
    ...
    function setBorrowTicketContract(address _contract) external onlyOwner {
        ...
        // @audit check for _contract != address(0) is missing
        // @audit what if the owner changes the borrowTicketContract?
        // Users who have set their NFT as collateral will get stuck in this contract
    ...
    function withdrawOriginationFees(address asset, uint256 amount, address to) external onlyOwner {
        // @audit Where is the list of assets for provided fee?
    ...
    function _interestOwed(
        ...
        // x = loanAmount * timeLent * interestRate * 10^18 / 365 days
        // interest = x / (10^21)
        // so,
        // interest = loanAmount * (timeLent/365 days) * (interestRate/10^3)
        // totalInterest = interest + accumulatedInterest
        // 31536000
        return
            (loanAmount * (block.timestamp - lastAccumulatedTimestamp) * ((perAnumInterestRate * 1e18) / 365 days)) /
            1e21 + // SCALAR * 1e18
            accumulatedInterest;
    }
}
